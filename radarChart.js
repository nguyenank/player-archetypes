...
//// Modified by An Nguyen //////////////////////////////
	...
	function layout(radarChartOptions) {
		...
		d3.csv("archetypes-data.csv", function(data) {
			firstRows = [transformRow(data[0]), transformRow(null)];
			...
			function playerInfo(id, player) {
				var pS = d3.select(id);
				pS.select(".player-name").text(player.Player);
			...
			let elem = document.querySelector("#left-player");
			let rect = elem.getBoundingClientRect();

			playerInfo("#left-player", data[0]);
			...
			function update() {
				...
				rows = [transformRow(data[selectedLeft]), transformRow(data[selectedRight])];
...
function RadarChart(id, data, options) {
	...
	var actualMax = d3.max(data, function(i){return d3.max(i.map(function(o){return o.value;}))});
	if (actualMax < 0.1) {
		maxValue = 0.2;
	}
	...
	axisGrid.selectAll(".axisLabel")
	   ...
	   .attr("fill", "#737373")
	   .text(function(d,i) { return Format((maxValue/cfg.levels)*(cfg.levels-i)); });
